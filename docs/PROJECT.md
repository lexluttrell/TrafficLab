...
## Mission 004 — 2026-09-26

Implemented the authorized ramp/boundary phase; see [run notes](MISSION-004.md) and [hosted experiment](ramps.html). Expanded frozen network, explicit connected ramp bindings, five measured inputs and three bounded inferred exits. Same-network through-only control and low/high exit assumptions share mainline departure schedules. Only 18 eastbound station-bins remain independent of input/flow-balance estimation. Central run inserts 13,926/14,407; two eastbound merges leave 481 outside the model. This is a visible failure gate before calibration, not a reconstructed historical morning. Source quality, missing exits, provisional bindings and unmodeled downstream queues remain explicit. Sunday remains unused.
